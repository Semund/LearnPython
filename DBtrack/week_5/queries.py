...
from DBtrack.week_5.db import db_session
from model import Salary


def top_salary(num_rows=10):
    top_salary = Salary.query.order_by(Salary.salary.desc()).limit(num_rows)

    for s in top_salary:
        print(f'З/п: {s.job} - {s.salary}')


def salary_by_city(city_name):
    salary = Salary.query.filter(Salary.city == city_name).order_by(Salary.salary.desc())
...
def top_salary_by_domain(domain, num_rows=10):
...
def top_avg_salary_by_city(num_rows=5):
    ...
    for city, salary in top_salary:
        print(f'В городе {city} средняя зарплата {salary:.2f}')


if __name__ == '__main__':
    # top_salary(num_rows=5)
    # salary_by_city('Владикавказ')
    # top_salary_by_domain('@rambler.ru')
    average_salary()
    count_distinct_cities()
    top_avg_salary_by_city()